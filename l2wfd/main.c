...
main(int argc, char **argv)
{
	...
	int port = 8100;
	if (filter_srv_run(port) == -1)
		rte_exit(EXIT_FAILURE, "Cannot start filter rules server\n");
#endif
...
#ifdef USE_PACKET_FILTER
		filter_store_callback(portid,
							  rte_eth_add_rx_callback(portid, 0, filter_pckts_cb, NULL));
#endif
	}
